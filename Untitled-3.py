...
axs[1, 1].imshow(trial_test, aspect='auto', cmap='Greens');
axs[1, 1].set_xlabel('Bin');
axs[1, 1].set_ylabel('Neuron');
axs[1, 1].set_title('Condition Dependent Average');
# %%
list_to_sorted_arr(stim_onset_t, new_sorted_trials) - list_to_sorted_arr(stim_offset_t, new_sorted_trials)
for trial in range(NUM_TRIALS):
    window = [0, int(sorted_int_durations[trial])]
    BIN_SIZE = 10
    num_neurons = 2
    ...
    for neuron in range(num_neurons):
        ...
        for num, val in enumerate(new_sorted_trials):
            ...
            aligned_spikes = np.array(spike_times[neuron]) - stim_onset_t[val]
            trial_index_spikes = np.intersect1d(np.where(aligned_spikes <= window[1])[0], np.where(aligned_spikes > window[0])[0])
            spike_times_in_window = aligned_spikes[trial_index_spikes]
            binned_counts = bin_values_by_range(values= spike_times_in_window, start_values= bins, end_values= bins + BIN_SIZE)
            binned_counts_array.append(np.array(binned_counts))
        ...
        stacked_bin_array = np.vstack(binned_counts_array)
        sum_over_bins, _ = sum_over_trial_bins(stacked_bin_array, cumulative_sum_list)


    # binned_counts = bin_values_by_range(values = )
    # raw_bins,_,_,_,_,_,_ = rasters_and_psths(color = 'Greens', data = stim_onset_t, data_points= _, num_neurons= num_neurons, window= window, BIN_SIZE= BIN_SIZE, new_sorted_trials=new_sorted_trials, cumulative_sum_list= cumulative_sum_list)
        # print(sum_over_bins.shape)
# %%
